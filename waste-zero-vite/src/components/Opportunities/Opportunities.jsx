...
const Opportunities = () => {
  ...
  const [remoteOpps, setRemoteOpps] = useState(null);
  ...
  // Try using remote opportunities if available
  const displayedOpportunities = remoteOpps || opportunities;
  ...
  useEffect(() => {
    const fetchOpps = async () => {
      try {
        ...
        if (Array.isArray(data)) setRemoteOpps(data);
      } catch (e) {
        console.warn('Could not load remote opportunities:', e);
      }
    };
    fetchOpps();
  }, []);
